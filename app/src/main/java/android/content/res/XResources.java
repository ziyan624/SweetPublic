...
import com.android.love.sweet.SweetBridge;
import com.android.love.sweet.WC_MethodCook;
import com.android.love.sweet.WC_MethodCook.MethodHookParam;
import com.android.love.sweet.SweetBridge.CopyOnWriteSortedSet;
import com.android.love.sweet.callbacks.WC_LayoutInflated;
import com.android.love.sweet.callbacks.WC_LayoutInflated.LayoutInflatedParam;
import com.android.love.sweet.callbacks.WCallback;
import sweet.dummy.XResourcesSuperClass;
import sweet.dummy.XTypedArraySuperClass;

import static com.android.love.sweet.SweetHelpers.decrementMethodDepth;
import static com.android.love.sweet.SweetHelpers.findAndHookMethod;
import static com.android.love.sweet.SweetHelpers.getIntField;
import static com.android.love.sweet.SweetHelpers.getLongField;
import static com.android.love.sweet.SweetHelpers.getObjectField;
import static com.android.love.sweet.SweetHelpers.incrementMethodDepth;
...
public class XResources extends XResourcesSuperClass {
	...
	private static final SparseArray<HashMap<String, CopyOnWriteSortedSet<WC_LayoutInflated>>> sLayoutCallbacks = new SparseArray<>();
	...
	private static String getPackageName(String resDir) {
		...
		if (pkgInfo != null && pkgInfo.packageName != null) {
			Log.w(SweetBridge.TAG, "Package name for " + resDir + " had to be retrieved via parser");
	...
	public static void init(ThreadLocal<Object> latestResKey) throws Exception {
		...
		findAndHookMethod(LayoutInflater.class, "inflate", XmlPullParser.class, ViewGroup.class, boolean.class, new WC_MethodCook() {
			@Override
			protected void afterHookedMethod(MethodHookParam param) throws Throwable {
				...
				if (details != null) {
					...
					WCallback.callAll(liparam);
				}
			}
		});

		final WC_MethodCook parseIncludeHook = new WC_MethodCook() {
			@Override
	 ...
	 * {@link ISweetCookZygoteInit#initZygote initZygote()} for all apps. As there is no
			...
			if (incrementMethodDepth("getDrawable") == 1) {
				...
				if (replacement instanceof DrawableLoader) {
					try {
						...
						if (result != null)
							return result;
					} catch (Throwable t) { SweetBridge.log(t); }
				} else if (replacement instanceof Integer) {
	...
	public XmlResourceParser getLayout(int id) throws NotFoundException {
		...
		HashMap<String, CopyOnWriteSortedSet<WC_LayoutInflated>> inner;
		synchronized (sLayoutCallbacks) {
			inner = sLayoutCallbacks.get(id);
		}
		if (inner != null) {
			CopyOnWriteSortedSet<WC_LayoutInflated> callbacks;
			synchronized (inner) {
			...
			if (callbacks != null) {
				...
				if (value.type == TypedValue.TYPE_STRING) {
					...
					if (components.length == 3)
						...
						SweetBridge.log("Unexpected resource path \"" + value.string.toString()
								+ "\" for resource id 0x" + Integer.toHexString(id));
				} else {
					SweetBridge.log(new NotFoundException("Could not find file name for resource id 0x") + Integer.toHexString(id));
				}
	...
	private static int translateResId(int id, XResources origRes, Resources repRes) {
		try {
			...
			if (!repResDefined && origResId == 0 && !entryType.equals("id")) {
				SweetBridge.log(entryType + "/" + entryName + " is neither defined in module nor in original resources");
				return 0;
			}
	...
	private static int translateAttrId(String attrName, XResources origRes) {
		...
		try {
			...
			SweetBridge.log("Attribute " + attrName + " not found in original resources");
		}
		return origAttrId;
	}
	...
	private class XMLInstanceDetails {
		...
		public final CopyOnWriteSortedSet<WC_LayoutInflated> callbacks;
		public final XResources res = XResources.this;

		private XMLInstanceDetails(ResourceNames resNames, String variant, CopyOnWriteSortedSet<WC_LayoutInflated> callbacks) {
	 ...
	 */
	public WC_LayoutInflated.UnCook hookLayout(int id, WC_LayoutInflated callback) {
	 ...
	 * Hook the inflation of a layout.
	 *
	 * @deprecated Use {@link #hookLayout(String, String, String, WC_LayoutInflated)} instead.
	 *
	 ...
	 */
	@Deprecated
	public WC_LayoutInflated.UnCook hookLayout(String fullName, WC_LayoutInflated callback) {
		int id = getIdentifier(fullName, null, null);
		if (id == 0)
			throw new NotFoundException(fullName);
	 ...
	 * @return An object which can be used to remove the callback again.
	 */
	public WC_LayoutInflated.UnCook hookLayout(String pkg, String type, String name, WC_LayoutInflated callback) {
		int id = getIdentifier(name, type, pkg);
		if (id == 0)
			throw new NotFoundException(pkg + ":" + type + "/" + name);
		return hookLayout(id, callback);
	 ...
	 * See {@link #hookSystemWideLayout(String, String, String, WC_LayoutInflated)}.
	 *
	 * @param id The ID of the resource which should be replaced.
	 * @param callback The callback to be executed when the layout has been inflated.
	 * @return An object which can be used to remove the callback again.
	 */
	public static WC_LayoutInflated.UnCook hookSystemWideLayout(int id, WC_LayoutInflated callback) {
		if (id >= 0x7f000000)
			throw new IllegalArgumentException("ids >= 0x7f000000 are app specific and cannot be set for the framework");
	 ...
	 * See {@link #hookSystemWideLayout(String, String, String, WC_LayoutInflated)}.
	 *
	 * @deprecated Use {@link #hookSystemWideLayout(String, String, String, WC_LayoutInflated)} instead.
	 *
	 ...
	 */
	@Deprecated
	public static WC_LayoutInflated.UnCook hookSystemWideLayout(String fullName, WC_LayoutInflated callback) {
		int id = getSystem().getIdentifier(fullName, null, null);
		if (id == 0)
			throw new NotFoundException(fullName);
	 ...
	 * @return An object which can be used to remove the callback again.
	 */
	public static WC_LayoutInflated.UnCook hookSystemWideLayout(String pkg, String type, String name, WC_LayoutInflated callback) {
		int id = getSystem().getIdentifier(name, type, pkg);
		if (id == 0)
			throw new NotFoundException(pkg + ":" + type + "/" + name);
		return hookSystemWideLayout(id, callback);
	...
	private static WC_LayoutInflated.UnCook hookLayoutInternal(String resDir, int id, ResourceNames resNames, WC_LayoutInflated callback) {
		if (id == 0)
			throw new IllegalArgumentException("id 0 is not an allowed resource identifier");
		...
		return callback.new UnCook(resDir, id);
	}

	/** @hide */
	public static void unhookLayout(String resDir, int id, WC_LayoutInflated callback) {
		HashMap<String, CopyOnWriteSortedSet<WC_LayoutInflated>> inner;
		synchronized (sLayoutCallbacks) {
			inner = sLayoutCallbacks.get(id);
			if (inner == null)
